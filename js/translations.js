window.translations = {
  es: {
    nav: {
      home: "Inicio",
      about: "Sobre mí",
      projects: "Proyectos",
      contact: "Contacto",
    },
    home: {
      eyebrow: "Desarrollador de Software",
      title: "Erick Salmeron",
      subtitle:
        "Hola, soy desarrollador de software, me especializo en Frontend, pero también puedo trabajar con diversas tecnologías y librerías.",
      cta: "Ver proyectos",
      cvBtn: "Descargar CV",
      terminalLines: [
        "$ whoami",
        "erick_salmeron — software dev",
        "$ cat skills.txt",
        "JavaScript · React · CSS · HTML",
        "$ status",
        "disponible para nuevos proyectos ✓",
      ],
    },
    about: {
      title: "Sobre mí",
      lead:
        "Soy desarrollador de software en Cuautla, Morelos. Llevo más de 2 años transformando diseños en productos digitales funcionales, rápidos y agradables de usar.",
      body:
        "Mi trabajo combina atención al detalle visual con preocupación real por el rendimiento y la accesibilidad. Disfruto los retos donde el diseño y la ingeniería se encuentran: animaciones fluidas, layouts responsivos y componentes reutilizables que escalan.",
      skillsTitle: "Tecnologías",
      skills: ["React", "JavaScript", "CSS", "Node.js", "PHP", "Python", "Git", "Linux"],
      factsTitle: "Datos rápidos",
      facts: [
        { label: "Años de experiencia", value: "2+" },
        { label: "Proyectos entregados", value: "6" },
        { label: "Café por semana", value: "∞" },
      ],
    },
    projects: {
      title: "Proyectos",
      lead: "Una selección de trabajos recientes.",
      viewLabel: "Ver proyecto",
      githubLabel: "GitHub",
      items: [
        {
          name: "Portfolio V2",
          description:
            "Segunda versión de mi portafolio personal.",
          tags: ["Javascript", "CSS", "React"],
          url: "https://portfoliov2-rho-ashy.vercel.app/",
          github: "https://github.com/ericklgs/Portfoliov2/",
        },
        {
          name: "Portfolio V1",
          description:
            "Primera versión de mi portafolio personal.",
          tags: ["Javascript", "CSS", "React"],
          url: "https://myportfolio-ericklgs.vercel.app/",
          github: "https://github.com/ericklgs/myportfolio",
        },
        {
          name: "To Do app",
          description:
            "Aplicación simple para lista de tareas y recordatorios.",
          tags: ["CSS", "React"],
          url: "https://to-do-list-ericklgs.vercel.app/",
          github: "https://github.com/ericklgs/ToDoList",
        },
        {
          name: "Bank interface",
          description:
            "Sitio de interfaz para aplicación de banco, la cual fue construida con styled-compnents.",
          tags: ["CSS", "React", "Styled-components"],
          url: "https://styled-components-wheat.vercel.app/",
          github: "https://github.com/ericklgs/styled-components",
        },
      ],
    },
    contact: {
      title: "Contacto",
      lead:
        "¿Quieres platicar? ¡Escríbeme!",
      formNameLabel: "Nombre",
      formEmailLabel: "Correo electrónico",
      formMessageLabel: "Mensaje",
      formNamePlaceholder: "Tu nombre",
      formEmailPlaceholder: "tu@correo.com",
      formMessagePlaceholder: "Cuéntame sobre tu proyecto...",
      submit: "Enviar mensaje",
      submitted: "¡Gracias! Tu mensaje fue enviado",
      directTitle: "Contacto directo",
      directLabels: {
        email: "Correo",
        location: "Ubicación",
        social: "Redes",
      },
    },
    footer: {
      rights: "Todos los derechos reservados.",
      builtWith: "Hecho con ❤️.",
    },
  },
  en: {
    nav: {
      home: "Home",
      about: "About",
      projects: "Projects",
      contact: "Contact",
    },
    home: {
      eyebrow: "Sofware Developer",
      title: "Erick Salmeron",
      subtitle:
        "Hello, I'm a software developer. I specialize in Frontend, but I can also work with various technologies and libraries.",
      cta: "View projects",
      cvBtn: "Download CV",
      terminalLines: [
        "$ whoami",
        "erick_salmeron — software dev",
        "$ cat skills.txt",
        "JavaScript · React · CSS · HTML",
        "$ status",
        "available for new projects ✓",
      ],
    },
    about: {
      title: "About me",
      lead:
        "I'm a frontend developer based in Cuautla, Morelos. For over 2 years I've been turning designs into digital products that are fast, functional, and pleasant to use.",
      body:
        "My work blends attention to visual detail with real concern for performance and accessibility. I enjoy the spaces where design and engineering meet: smooth animations, responsive layouts, and reusable components that scale.",
      skillsTitle: "Technologies:",
      skills: ["React", "JavaScript", "CSS", "Node.js", "PHP", "Python", "Git", "Linux"],
      factsTitle: "Quick facts",
      facts: [
        { label: "Years of experience", value: "2+" },
        { label: "Projects shipped", value: "6" },
        { label: "Coffee per week", value: "∞" },
      ],
    },
    projects: {
      title: "Projects",
      lead: "A selection of recent work.",
      viewLabel: "View project",
      githubLabel: "GitHub",
      items: [
        {
          name: "Portfolio V2",
          description:
            "Second version of my personal portfolio.",
          tags: ["Javascript", "CSS", "React"],
          url: "https://portfoliov2-rho-ashy.vercel.app/",
          github: "https://github.com/ericklgs/Portfoliov2/",
        },
        {
          name: "Portfolio V1",
          description:
            "First version of my personal portfolio.",
          tags: ["Javascript", "CSS", "React"],
          url: "https://myportfolio-ericklgs.vercel.app/",
          github: "https://github.com/ericklgs/myportfolio",
        },
        {
          name: "To Do app",
          description:
            "Simple To Do list app for reminders and tasks.",
          tags: ["CSS", "React"],
          url: "https://to-do-list-ericklgs.vercel.app/",
          github: "https://github.com/ericklgs/ToDoList",
        },
        {
          name: "Bank interface",
          description:
            "Bank app interface, built using styled-components.",
          tags: ["CSS", "React", "Styled-components"],
          url: "https://styled-components-wheat.vercel.app/",
          github: "https://github.com/ericklgs/styled-components",
        },
      ],
    },
    contact: {
      title: "Contact",
      lead:
        "Want to chat? Send me a message!",
      formNameLabel: "Name",
      formEmailLabel: "Email",
      formMessageLabel: "Message",
      formNamePlaceholder: "Your name",
      formEmailPlaceholder: "you@email.com",
      formMessagePlaceholder: "Tell me about your project...",
      submit: "Send message",
      submitted: "Thanks! Your message was sent",
      directTitle: "Direct contact",
      directLabels: {
        email: "Email",
        location: "Location",
        social: "Social",
      },
    },
    footer: {
      rights: "All rights reserved.",
      builtWith: "Made with ❤️.",
    },
  },
};

